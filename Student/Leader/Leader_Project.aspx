<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Leader_Project.aspx.cs" Inherits="Project_Board.Student.Leader.Leader_Project" MasterPageFile="~/Student/Leader/Leader.master" %>
                    ...
                    <asp:Repeater ID="rptProposals" runat="server" OnItemCommand="rptProposals_ItemCommand">
                        <ItemTemplate>
                            <div class="proposal-card">
                                ...
                                <%# !string.IsNullOrWhiteSpace(Convert.ToString(Eval("Keywords"))) ? "<div style='margin-bottom:0.75rem;'>" + string.Join("", Eval("Keywords").ToString().Split(',').Select(k => "<span class=\"tag-pill\"><i class=\"fa-solid fa-tag\" style=\"font-size:0.7rem; margin-right:0.3rem;\"></i>" + HttpUtility.HtmlEncode(k.Trim()) + "</span>")) + "</div>" : "" %>
                                ...
                                <div style="font-size:0.78rem; color:var(--c-text-dim); margin-top:0.75rem;">
                                    Submitted on: <%# Eval("SubmittedAt") != DBNull.Value ? Convert.ToDateTime(Eval("SubmittedAt")).ToString("MMM dd, yyyy hh:mm tt") : "Not recorded" %>
                                </div>
                            </div>
                        </ItemTemplate>
                        <FooterTemplate>
