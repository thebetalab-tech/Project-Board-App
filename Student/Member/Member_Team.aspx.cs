...
namespace Project_Board.Student.Member
{
    public partial class Member_Team : Page
    {
        ...
        protected string LeaderEmail { get; set; } = "";
        ...
        private void LoadTeamDetails()
        {
            ...
            using (SqlConnection conn = new SqlConnection(ConnString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            ...
                            TechName = reader["TechName"] != DBNull.Value ? reader["TechName"].ToString() : "General";
                            LeaderName = reader["LeaderName"] != DBNull.Value ? reader["LeaderName"].ToString() : "Not Assigned";
                            LeaderEmail = reader["LeaderEmail"] != DBNull.Value ? reader["LeaderEmail"].ToString() : "";
                ...
                if (IsAssigned && groupId > 0)
                {
                    pnlAssigned.Visible = true;
                    pnlUnassigned.Visible = false;

                    // Configure Mentor Status Panels
                    if (IsMentorAssigned)
                    {
                        pnlMentorAssigned.Visible = true;
                        pnlMentorPending.Visible = false;
                        pnlMentorNone.Visible = false;
                    }
                    else if (IsMentorPending)
                    {
                        pnlMentorAssigned.Visible = false;
                        pnlMentorPending.Visible = true;
                        pnlMentorNone.Visible = false;
                    }
                    else
                    {
